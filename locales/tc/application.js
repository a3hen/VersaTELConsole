...
module.exports = {
  ...
  TIP_APP_TYPE:
    'CoSAN Manager 支持來自於應用商店和應用倉庫的應用部署(基於 Helm)，同样也支持自制應用(Application CRD)。',
  TIP_APP_GOVERNANCE:
    '使用應用治理需要創建自制應用並對每項服務開啟服務治理功能',
  APP_REPOS_DESC:
    '應用倉庫來自於第三方的 Helm Chart Repo，通過視覺化的方式在 CoSAN Manager 中展示並提供部署及管理功能，用戶可以基於應用倉庫中的模板快速地一鍵部署應用。',
