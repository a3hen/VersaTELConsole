...
export default class RemoteBackupStore extends Base {
  ...
  getListUrl = this.getRemoteBackupingUrl

  constructor(module = 'remotebackup') {
    super(module)
  }
  ...
  async fetchList({
    ...
    const result = await request.get(this.getRemoteBackupUrl(), {
      ...params,
    })
  ...
  @action
  async fetchLRemoteBackupTemplates() {
    this.RemoteBackupTemplates.isLoading = true

    const result = await request.get(
      `/kapis/versatel.kubesphere.io/v1alpha1/remote`
    )
    this.RemoteBackupTemplates.update({
  ...
  async fetchDetail(params) {
    ...
    const detail = { ...params, ...data[0], kind: 'RemoteBackup' }
    this.detail = detail
