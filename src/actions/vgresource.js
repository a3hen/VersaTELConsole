...
import CreateModal from 'components/Modals/VGCreate'
...
export default {
    ...
    on({ store, cluster, namespace, workspace, success, devops, ...props }) {
      ...
      const modal = Modal.open({
        onOk: data => {
          ...
          request
            .post(
              `/kapis/versatel.kubesphere.io/v1alpha1/vg`,
              data
            )
            .then(res => {
              // Modal.close(modal)
    ...
    on({ store, detail, success, ...props }) {
      const modal = Modal.open({
        onOk: () => {
          request
            .delete(
              `/kapis/versatel.kubesphere.io/v1alpha1/vg/${detail.node}/${detail.vg}`
            )
            .then(res => {
              // Modal.close(modal)
              if (res !== "删除成功") {
  ...
  'vgresource.batch.delete': {
    on({ store, success, rowKey, nodename, ...props }) {
      const { data, selectedRowKeys } = store.list
      const selectValues = data
        .filter(item => selectedRowKeys.includes(item["vg"]) && item.node === nodename)
        .map(item => {
          return { name: item.vg, node: item.node }
        })
        ...
        onOk: async () => {
          ...
          data.forEach(item => {
            const selectValue = selectValues.find(
              value =>
                value.name === item.vg && value.node === item.node
            )

            if (selectValue) {
              reqs.push(request.delete(`/kapis/versatel.kubesphere.io/v1alpha1/vg/${selectValue.node}/${selectValue.name}`))
            }
          })
